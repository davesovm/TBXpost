"""
bot/commands.py — All Telegram bot command handlers.

Commands:
    /log [message]  — save a dev update to local SQLite
    /status         — show last 5 dev logs
    /generate       — generate a post from recent logs
    /post_now       — post latest generated post to the channel
    /news           — fetch top news and post it immediately
"""
...
from core.engine import GeneratorError
...
async def cmd_log(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /log [message] — Save a dev update to the local database.
    Usage: /log finished the OAuth module, fixed rate limiting
    """
    ...
    log.info("Log saved: id=%d", row_id)
    await update.message.reply_text(f"✅ Logged (#{row_id}): {text[:80]}")


async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /status — Show the last 5 dev logs from local database.
    """
    cfg: Config = context.bot_data["cfg"]
    if not _is_admin(update, cfg):
        await update.message.reply_text("⛔ Unauthorised.")
    ...
    lines = ["📋 Last 5 logs:\n"]
    for entry in logs:
        lines.append(f"[{entry['created_at'][:10]}] {entry['message'][:100]}")
    await update.message.reply_text("\n".join(lines))


async def cmd_generate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /generate [tone] — Generate a post from recent logs using AI.
    Tones: builder (default), humorous, technical
    """
    cfg: Config = context.bot_data["cfg"]
    if not _is_admin(update, cfg):
        await update.message.reply_text("⛔ Unauthorised.")
    ...
    try:
        content, score = engine.from_logs(
            logs=logs, tone=tone,
            groq_api_key=cfg.groq_api_key, groq_model=cfg.groq_model,
            groq_api_url=cfg.groq_api_url,
            gemini_api_key=cfg.gemini_api_key, gemini_model=cfg.gemini_model,
        )
    ...
    log.info("Post generated: id=%d score=%d", post_id, score)
    await update.message.reply_text(
...
async def cmd_post_now(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not post:
        await update.message.reply_text("No posts yet. Run /generate first.")
        return

    success, msg_id, post_url = tg.send_message(
        cfg.telegram_bot_token, cfg.telegram_channel, post["content"]
    )
    db.record_posting(
        cfg.db_path, post_id=post["id"], platform="telegram",
        success=success, message_id=msg_id, post_url=post_url,
...
async def cmd_news(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /news — Fetch the top RSS article, generate a post, and publish it.
    """
    cfg: Config = context.bot_data["cfg"]
    if not _is_admin(update, cfg):
        await update.message.reply_text("⛔ Unauthorised.")
    ...
    try:
        content, score = engine.from_news(
            title=article["title"], desc=article["desc"], url=article["url"],
            tone="builder",
            groq_api_key=cfg.groq_api_key, groq_model=cfg.groq_model,
            groq_api_url=cfg.groq_api_url,
            gemini_api_key=cfg.gemini_api_key, gemini_model=cfg.gemini_model,
    ...
    if score < cfg.news_min_score:
        await update.message.reply_text(
            f"⚠️ Score {score}/10 is below threshold {cfg.news_min_score}. Not posting."
        )
        return
    ...
    db.record_posting(
        cfg.db_path, post_id=post_id, platform="telegram",
        success=success, message_id=msg_id, post_url=post_url,
    ...
    if success:
        ...
        await update.message.reply_text("❌ Posting failed. Content was saved locally.")
